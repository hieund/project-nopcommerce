    ...
    public partial class ProductController : BaseNopController
    {
        ...
        public ActionResult Edit(ProductModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                UpdateProductDetail(product, model);
                //Remove cache
                foreach (var pc in product.ProductCategories)
                {
                    RemoveCache("Category_Cache_" + pc.CategoryId);
                }
                RemoveCache("RecentlyAddedProducts");
        ...
        private void UpdateProductDetail(Product product, ProductModel model)
        {
            if (Request["hdfIsProductDetailLoaded"] == "1")
            {
                ...
                if (category != null)
                {
                    ...
                    foreach (ProductProperty property in lstProperties)
                    {
                        ProductDetail pd = lstProductDetails.FirstOrDefault(x => x.PropertyId == property.Id);
                        if (pd == null)
                        {
                            pd = new ProductDetail();
                            pd.PropertyId = property.Id;
                            pd.PropertyType = property.Type;
                            pd.ProductId = product.Id;
                            pd.GroupId = property.GroupId;
                            pd.CategoryId = category.CategoryId;
                            if (Request.Unvalidated.Form["datactl-" + property.Id] != null)
                            {
                                pd.Value = Request.Unvalidated.Form["datactl-" + property.Id];
                                _productService.InsertProductDetail(pd);
                            }
                        }
                        else
                        {
                            ...
                            {
                                pd.Value = Request.Unvalidated.Form["datactl-" + property.Id];
                                _productService.UpdateProductDetail(pd);
                            }
                        }
                    }
                    GenerateHtml(product.Id);
                }
            }
        }
        ...
        [HttpPost, GridAction(EnableCustomBinding = true)]
        public ActionResult PromotionList(GridCommand command, PromotionListModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();

            var promotions = _promotionService.GetAllPromo(model.SearchPromotionName,
                command.Page - 1, command.PageSize);
            var gridModel = new GridModel<PromotionModel>
            {
                Data = promotions.Select(x =>
                {
                    var promotionModel = x.ToModel();
        ...
        public ActionResult PromotionDeleteSelected(string selectedIds)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
            ...
            if (selectedIds != null)
            {
                var ids = selectedIds
                    ...
                    .Select(x => Convert.ToInt32(x))
                    .ToArray();
                promotions.AddRange(_promotionService.GetPromotionByIds(ids));
        ...
        #region External Form Attribute
        ...
        [HttpPost, ParameterBasedOnFormNameAttribute("save-continue", "continueEditing")]
        public ActionResult CreatePromotion(PromotionModel model, bool continueEditing)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
            ...
            if (ModelState.IsValid)
            {
                ...
                promotion.StartDate = model.StartDate.Value;
                promotion.TotalAmount = model.TotalAmount;
                _promotionService.InsertPromotion(promotion);
                SuccessNotification(_localizationService.GetResource("Admin.Catalog.Promotion.Added"));
                return continueEditing ? RedirectToAction("Edit", new { id = promotion.Id }) : RedirectToAction("PromotionList");
            }
        ...
        public ActionResult EditPromotion(int id)
        {
            ...
            var promo = _promotionService.GetPromotionById(id);
            if (promo == null || promo.Deleted)
                //No category found with the specified id
                return RedirectToAction("PromotionList");

            var model = promo.ToModel();
            //locales
            AddLocales(_languageService, model.Locales, (locale, languageId) =>
            {
                //locale.PromotionName = promo.GetLocalized(x => x.PromotionName, languageId, false, false);
                //locale.Description = promo.GetLocalized(x => x.Description, languageId, false, false);
                //locale.TotalAmount = promo.GetLocalized(x => x.TotalAmount, languageId, false, false);
                //locale.StartDate = promo.GetLocalized(x => x.StartDate, languageId, false, false);
                //locale.EndDate = promo.GetLocalized(x => x.EndDate, languageId, false, false);
            });
        ...
        [HttpPost, ParameterBasedOnFormNameAttribute("save-continue", "continueEditing")]
        public ActionResult EditPromotion(PromotionModel model, bool continueEditing)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();

            var promo = _promotionService.GetPromotionById(model.Id);
            if (promo == null || promo.Deleted)
                //No category found with the specified id
                return RedirectToAction("PromotionList");
            ...
            if (ModelState.IsValid)
            {
                //promo = model.ToEntity(promo);
                promo.UpdateDate = DateTime.UtcNow;
                promo.PromotionName = model.PromotionName;
                promo.Description = model.Description;
                promo.TotalAmount = model.TotalAmount;
                promo.StartDate = model.StartDate.Value;
                promo.EndDate = model.EndDate.Value;
                promo.Published = model.Published;
                promo.DisplayOrder = model.DisplayOrder;
                _promotionService.UpdatePromotion(promo);

                SuccessNotification(_localizationService.GetResource("Admin.Catalog.Promotion.Updated"));
                return continueEditing ? RedirectToAction("EditPromotion", promo.Id) : RedirectToAction("PromotionList");
            }

            return View(model);
        }
        [HttpPost, GridAction(EnableCustomBinding = true)]
        public ActionResult PromotionDetailList(GridCommand command, PromotionModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();

            var promotion = _promotionService.GetPromotionById(model.Id);
            if (promotion != null && promotion.Deleted == true)
            {
                return Content("This is not your promotion");
            }

            var promotiondetail = _promotionService.GetPromotionDetailByPromotion(model.Id, model.SearchProductName, model.SearchCategoryId, model.SearchManufacturerId);
            var PromotionDetailsModel = promotiondetail
                .Select(x =>
                {
                    return new PromotionDetailModel()
                    {
                        ...
                        ManufacturerName = x.ManufactureName
                    };
                })
                .ToList();

            var modelnew = new GridModel<PromotionDetailModel>
            {
                Data = PromotionDetailsModel,
                Total = PromotionDetailsModel.Count
            };
        ...
        [GridAction(EnableCustomBinding = true)]
        public ActionResult PromotionDetailDelete(int id, GridCommand command)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();

            var promotiondetail = _promotionService.GetPromotionDetailById(id);
            if (promotiondetail == null)
                throw new ArgumentException("No promotion product found with the specified id");

            var productId = promotiondetail.ProductId;
            ...
            _promotionService.DeletePromotionDetail(promotiondetail);
            var promotionmodel = new PromotionModel();
            //categories
            promotionmodel.AvailableCategories.Add(new SelectListItem() { Text = _localizationService.GetResource("Admin.Common.All"), Value = "0" });
            foreach (var c in _categoryService.GetAllCategories(showHidden: true))
                promotionmodel.AvailableCategories.Add(new SelectListItem() { Text = c.GetCategoryNameWithPrefix(_categoryService), Value = c.Id.ToString() });

            //manufacturers
            promotionmodel.AvailableManufacturers.Add(new SelectListItem() { Text = _localizationService.GetResource("Admin.Common.All"), Value = "0" });
            foreach (var m in _manufacturerService.GetAllManufacturers(true))
                promotionmodel.AvailableManufacturers.Add(new SelectListItem() { Text = m.Name, Value = m.Id.ToString() });
            return PromotionDetailList(command, promotionmodel);
        }
        public ActionResult PromotionDetailDeleteSelected(string selectedIds)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();

            var promodetails = new List<PromotionDetail>();
            int promotionid = 0;
            if (selectedIds != null)
            {
                var ids = selectedIds
                ...
                if (ids != null && ids.Count() > 0)
                {
                    ...
                    promodetails.AddRange(_promotionService.GetPromotionDetailByIds(ids.Skip(1).ToArray()));

                    for (int i = 0; i < promodetails.Count; i++)
                    {
                        var promodetail = promodetails[i];
                        _promotionService.DeletePromotionDetail(promodetail);
                    }
                }
            }
            return promotionid != 0 ? RedirectToAction("EditPromotion", new { id = promotionid }) : RedirectToAction("PromotionList");
        }
        public ActionResult PromotionProductAddPopup(int promotionId, string promotionName)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();

            var model = new ProductModel.AddPromotionProductModel();
        ...
        [HttpPost, GridAction(EnableCustomBinding = true)]
        public ActionResult PromotionProductAddPopupList(GridCommand command, ProductModel.AddPromotionProductModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
        ...
        public ActionResult PromotionProductAddPopup(string btnId, string formId, ProductModel.AddPromotionProductModel model, FormCollection collection)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
                ...
                if (model.SelectedProductIds != null)
                {
                    foreach (int id in model.SelectedProductIds)
                    {
                        ...
                        if (product != null)
                        {
                            ...
                            if (!_promotionService.CheckPromotionDetailExist(model.PromotionId, id))
                            {
                                var productManufacturers = _manufacturerService.GetProductManufacturersByProductId(product.Id, true);
                                var productCategories = _categoryService.GetProductCategoriesByProductId(product.Id, true);
                                if (productManufacturers != null && productManufacturers.FirstOrDefault() != null || productCategories != null && productCategories.FirstOrDefault() != null)
                                {
                                    _promotionService.InsertPromotionDetail(
                                        new PromotionDetail()
                                        {
                                            ...
                                            ManufactureId = productManufacturers.First().ManufacturerId,
                                            ManufactureName = productManufacturers.First().Manufacturer.Name,
                                            CategoryId = productCategories.First().CategoryId,
                                            CategoryName = productCategories.First().Category.Name
                                        });
                                }
                            }
                        }
            ...
            else if (collection["save-category"] != null)
            {
                List<int> lstcat = new List<int>();
                lstcat.Add(model.SearchCategoryId);
                var products = _productService.SearchProducts(0, 2147483647, lstcat, 0, 0, 0, null, null, null, 0, null, false, false, 0, null, ProductSortingEnum.Position, false);
                foreach (var product in products)
                {
                    ...
                    if (!_promotionService.CheckPromotionDetailExist(model.PromotionId, product.Id))
                    {
                        var productManufacturers = _manufacturerService.GetProductManufacturersByProductId(product.Id, true);
                        var productCategories = _categoryService.GetProductCategoriesByProductId(product.Id, true);
                        if (productManufacturers != null && productManufacturers.FirstOrDefault() != null || productCategories != null && productCategories.FirstOrDefault() != null)
            ...
            else if (collection["save-manufacture"] != null)
            {
                var products = _productService.SearchProducts(0, 2147483647, null, model.SearchManufacturerId, 0, 0, null, null, null, 0, null, false, false, 0, null, ProductSortingEnum.Position, false);
                foreach (var product in products)
                {
                    //a vendor should have access only to his products
                    if (_workContext.CurrentVendor != null && product.VendorId != _workContext.CurrentVendor.Id)
            ...
            else if (collection["del-category"] != null)
            {
                List<int> lstcat = new List<int>();
                lstcat.Add(model.SearchCategoryId);
                var products = _productService.SearchProducts(0, 2147483647, lstcat, 0, 0, 0, null, null, null, 0, null, false, false, 0, null, ProductSortingEnum.Position, false);
                foreach (var product in products)
                {
                    ...
                    if (_workContext.CurrentVendor != null && product.VendorId != _workContext.CurrentVendor.Id)
                        continue;
                    var promodetails = _promotionService.GetPromotionDetailByProductId(model.PromotionId, product.Id);
            ...
            else if (collection["del-manufacture"] != null)
            {
                var products = _productService.SearchProducts(0, 2147483647, null, model.SearchManufacturerId, 0, 0, null, null, null, 0, null, false, false, 0, null, ProductSortingEnum.Position, false);
                foreach (var product in products)
                {
        ...
        /*[HttpPost]
        [FormValueRequired("save-category")]
        public ActionResult PromotionProductAddPopup(string btnId, string formId, ProductModel.AddPromotionProductModel model)
         {
             if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                 return AccessDeniedView();
            
             if (model.SearchCategoryId != null)
             {
                 List<int> lstcat = new List<int>();
                 lstcat.Add(model.SearchCategoryId);
                 var products = _productService.SearchProducts(0, 2147483647, lstcat, 0, 0, 0, null, null, null, 0, null, false, false, 0, null, ProductSortingEnum.Position, false);
         ...
         [HttpPost]
         [FormValueRequired("save-manufacure")]
         public ActionResult PromotionProductAddPopup(string btnId, string formId, ProductModel.AddPromotionProductModel model)
         {
             if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                 return AccessDeniedView();

             if (model.SearchManufacturerId != null)
             {
                 var products = _productService.SearchProducts(0, 2147483647, null, model.SearchManufacturerId, 0, 0, null, null, null, 0, null, false, false, 0, null, ProductSortingEnum.Position, false);
                 foreach (var product in products)
                 {
