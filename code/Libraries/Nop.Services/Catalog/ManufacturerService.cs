...
namespace Nop.Services.Catalog
{
    ...
    public partial class ManufacturerService : IManufacturerService
    {
        ...
        private const string CATEGORYMANUFACTURE_PATTERN_KEY = "Nop.manfacturer.bycategoryid-{0}";
        ...
        #region Category Manufacuture

        public virtual IList<Manufacturer> GetListManufacturerByCategoryId(int categoryId)
        {
            if (categoryId == 0)
                return new List<Manufacturer>();
            string key = string.Format(CATEGORYMANUFACTURE_PATTERN_KEY, categoryId);

            return _cacheManager.Get(key, () =>
                {
                    //SELECT FROM PRODUCT_MANUFACTURER, PRODUCT_CATEGORY, MANUFACTURER
                    var query = from manu in
                                    (from pc in _productcategoryRepository.Table
                                     ...
                                     where pc.CategoryId == categoryId && !m.Deleted
                                     orderby m.DisplayOrder
                                     select m)
                                group manu by manu.Id into lstm
                                select lstm;

                    var listmanufacture = query.ToList();

                    List<Manufacturer> lst = new List<Manufacturer>();
                    foreach (var manufacture in listmanufacture)
                    {
                        lst.Add(new Manufacturer
                            {
                                Id = manufacture.Key,
                                Name = manufacture.Select(m => m.Name).First(),
                                MetaTitle = manufacture.Select(m => m.MetaTitle).First(),
                                MetaDescription = manufacture.Select(m => m.MetaDescription).First(),
                                MetaKeywords = manufacture.Select(m => m.MetaKeywords).First()
                            });
                    }
                    return lst;
                });
