    ...
    public partial class ProductService : IProductService
    {
            ...
            bool showHidden = false)
        {
            ...
            if (_commonSettings.UseStoredProceduresIfSupported && _dataProvider.StoredProceduredSupported)
            {
                ...
                //check keywords to switch FullTextMode
                string[] sKeywords = string.IsNullOrEmpty(keywords) ? new string[] { } : keywords.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                ...
                if (sKeywords.Length > 1)
                    ...
                    pFullTextMode.Value = (int)FulltextSearchMode.ExactMatch;
        ...
        public virtual IList<Product> GetAllProductCollections()
        {
            var query = (from rp in _productRepository.Table.AsEnumerable()
                         join p in _crossSellProductRepository.Table on rp.Id equals p.ProductId1
                         where !rp.Deleted && rp.Published
                         select rp).Distinct();
            var products = query.ToList();
            return products;
        }
        ...
        public virtual IList<ProductCollection> GetProductsCollectionByProductId1(int productId1, bool showHidden = false)
        {
            var query = from rp in _productCollectionRepository.Table
                        join p in _productRepository.Table on rp.ProductId2 equals p.Id
                        where rp.ProductId1 == productId1 &&
                        !p.Deleted &&
                        (showHidden || p.Published)
                        orderby rp.DisplayOrder
                        select rp;
            var productCollections = query.ToList();
        ...
        public virtual ProductPropertyValue GetProductPropertyValue(int PropertyValueId)
        {
            if (PropertyValueId == 0)
                return null;

            return _productPropertyValueRepository.GetById(PropertyValueId);
        }
        public virtual ProductProperty GetProductProperty(int PropertyId)
        {
            if (PropertyId == 0)
                return null;

            return _productPropertyRepository.GetById(PropertyId);
        }
        #endregion
