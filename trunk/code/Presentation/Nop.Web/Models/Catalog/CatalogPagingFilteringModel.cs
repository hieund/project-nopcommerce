...
namespace Nop.Web.Models.Catalog
{
    public partial class CatalogPagingFilteringModel : BasePageableModel
    {
        ...
        public CatalogPagingFilteringModel()
        {
            ...
            this.PropertiesFilter = new PropertiesFilterModel();
        ...
        /// <summary>
        /// Properties filter model
        /// </summary>
        public PropertiesFilterModel PropertiesFilter { get; set; }
        ...
        public partial class PropertiesFilterModel : BaseNopModel
        {
            #region Const

            private const string QUERYSTRINGPARAM = "pro";

            #endregion

            #region Ctor

            public PropertiesFilterModel()
            {
                this.Items = new List<PropertieValuesFilterItem>();
            }
            ...
            protected virtual string GenerateFilteredPropertiesQueryParam(IList<string> options)
            {
                string result = "";
            ...
            public virtual List<string> GetAlreadyFilteredPropertiesOption(IWebHelper webHelper)
            {
            ...
            /// <param name="alreadyFilteredPropertiesValueOptions"></param>
            /// <param name="categoryservice"></param>
            /// <param name="webHelper"></param>
            /// <param name="workContext"></param>
            public virtual void PreparePropertiesFilters(int categoryId, IList<string> alreadyFilteredPropertiesValueOptions,
                ICategoryService categoryservice,
                IWebHelper webHelper,
                IWorkContext workContext)
            {
                var allFilters = new List<ProductPropertyValue>();
                var lstallpp = categoryservice.GetProductPropertiesByProductCategoryId(categoryId);
                var lstpp = lstallpp.Where(pp => pp.IsFilterExtension).ToList();

                foreach (var pp in lstpp)
                {
                    var lstppv = pp.ListProductPropertyValue;
                    if (lstppv.Count > 0)
                    {
                        foreach (var objppv in lstppv)
                        {
                            var flagselectd = alreadyFilteredPropertiesValueOptions.Where(v => v.ToLower() == objppv.Name.ToLower()).Count() > 0;
                            var newItem = new PropertieValuesFilterItem();
                            newItem = new PropertieValuesFilterItem();
                            newItem.PropertyId = pp.Id;
                            newItem.PropertyName = pp.Name;
                            newItem.Selected = flagselectd;
                            newItem.ValueId = objppv.Id;
                            newItem.ValueName = objppv.Name;

                            IList<string> lstfilter = new List<string>();
                            if (alreadyFilteredPropertiesValueOptions.Count > 0)
                            {
                                alreadyFilteredPropertiesValueOptions.Add(objppv.Name);
                                lstfilter = alreadyFilteredPropertiesValueOptions;
                            }
                            else
                            {
                                lstfilter.Add(objppv.Name);
                            }

                            string newQueryParam = GenerateFilteredPropertiescQueryParam(lstfilter);
                            string filterUrl = webHelper.ModifyQueryString(webHelper.GetThisPageUrl(true), QUERYSTRINGPARAM + "=" + newQueryParam, null);
                            filterUrl = ExcludeQueryStringParams(filterUrl, webHelper);
                            newItem.FilterUrl = filterUrl;
            ...
            protected virtual string GenerateFilteredPropertiescQueryParam(IList<string> optionIds)
            {
                string result = "";
            ...
            public IList<PropertieValuesFilterItem> Items { get; set; }
            public IList<PropertieValuesFilterItem> AlreadyFilteredItems { get; set; }
            public string RemoveFilterUrl { get; set; }

            #endregion
        }

        public partial class PropertieValuesFilterItem : BaseNopModel
        {
