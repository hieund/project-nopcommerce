...
namespace Nop.Web.Controllers
{
    public partial class CatalogController : BaseNopController
    {
        ...
        protected IEnumerable<ProductOverviewModel> PrepareProductOverviewModels(IEnumerable<Product> products,
            ...
            bool forceRedirectionAfterAddingToCart = false, bool isSearching = false)
        {
            if (products == null)
            ...
            foreach (var product in products)
            {
                var model = new ProductOverviewModel()
                {
                    ...
                    CreateDate = product.CreatedOnUtc
                        ...
                        default:
                            {
                                ...
                                if (_permissionService.Authorize(StandardPermissionProvider.DisplayPrices))
                                {
                                    ...
                                    var productVariant = _priceCalculationService.GetProductVariantWithMinimalPrice(productVariants, _workContext.CurrentCustomer, true, int.MaxValue, out minimalPrice, isSearching);
                                    if (productVariant == null)
                                        continue;
                                    priceModel.Id = productVariant.Id;
                ...
                if (model.ProductPrice.PriceValue > 0 || isSearching)
                    models.Add(model);
            }
            return models;
        }
        ...
        protected ProductDetailsModel PrepareProductDetailsPageModel(Product product)
        {
            if (product == null)
                throw new ArgumentNullException("product");
            var objpicture = product.GetDefaultProductPicture(_pictureService);
            var model = new ProductDetailsModel()
            {
                ...
                SeName = product.GetSeName(),
                HtmlSpec = product.GetLocalized(x => x.HtmlSpec),
                MetaOgDescription = string.Format("<meta property=\"og:description\" content=\"{0}\" />", product.GetLocalized(x => x.MetaDescription)),
                MetaOgTitle = string.Format("<meta property=\"og:title\" content=\"{0}\">", (!string.IsNullOrEmpty(product.GetLocalized(x => x.MetaTitle)) ? product.GetLocalized(x => x.MetaTitle) : product.GetLocalized(x => x.Name))),
                MetaOgImg = string.Format("<meta property=\"og:image\" content=\"{0}\" />", _pictureService.GetPictureUrl(objpicture, _mediaSettings.ProductThumbPictureSizeOnProductDetailsPage)),
                MetaOgUrl = string.Format("<meta property=\"og:url\" content=\"{0}\" />", Request.Url)
            };
            //template
            ...
            //product detail
            var productdetails = _productService.GetProductDetailByProductId(product.Id).OrderBy(p => p.GroupId).ThenBy(p => p.PropertyId);
            foreach (var productdetail in productdetails)
            {
                model.ProductConfigModels.Add(new ProductConfigModel()
                {
                    ProductId = productdetail.ProductId,
                    CategoryId = productdetail.CategoryId,
                    GroupId = productdetail.GroupId,
                    PropertyId = productdetail.PropertyId,
                    PropertyName = GetProductProperty(productdetail.PropertyId),
                    PropertyType = productdetail.PropertyType,
                    Value = GetProductPropertyValue(productdetail.Value)
                });
            }
            //product variants
            foreach (var variant in _productService.GetProductVariantsByProductId(product.Id))
            {
                ...
                PictureModel objpic = new PictureModel();
                objpic.ImageUrl = _pictureService.GetPictureUrl(variant.PictureId, _mediaSettings.ProductVariantPictureSize, false);
                objpic.FullSizeImageUrl = _pictureService.GetPictureUrl(variant.PictureId, 0, false);
                objpic.Title = variant.Name;
                objpic.ProductVariantId = variant.Id;
                if (objpic.ImageUrl != string.Empty)
                    model.PictureModels.Add(objpic);
            }

            return model;
        }
        protected string GetProductProperty(int propertyId)
        {
            var obj = _productService.GetProductProperty(propertyId);
            if (obj != null)
            {
                return obj.Name;
            }
        ...
        protected string GetProductPropertyValue(string strPropertyValueIds)
        {
            string result = "";
            string[] ids = strPropertyValueIds.Split(',');
            int id = 0;
            if (ids.Length > 0)
            {
                for (int i = 0; i < ids.Length; i++)
                {
                    if (int.TryParse(ids[0].ToString(), out id))
                    {
                        var obj = _productService.GetProductPropertyValue(id);
                        if (obj != null)
                        {
                            result = ", " + obj.Name;
                        }
            ...
            else if (int.TryParse(strPropertyValueIds, out id))
            {
                var obj = _productService.GetProductPropertyValue(id);
                if (obj != null)
                {
        ...
        protected void PrepareProductReviewsModel(ProductReviewsModel model, Product product)
        {
            ...
            foreach (var pr in productReviews)
            {
                ...
                model.Items.Add(new ProductReviewModel()
                {
                    ...
                    WrittenOnStr = _dateTimeHelper.ConvertToUserTime(pr.CreatedOnUtc, DateTimeKind.Utc).ToString("dd/MM/yyyy hh:mm"),
                });
            }
        ...
        protected ProductDetailsModel.ProductVariantModel PrepareProductVariantModel(ProductDetailsModel.ProductVariantModel model, ProductVariant productVariant)
        {
            ...
            model.Promotion = productVariant.Promotion;
            ...
            //set gia tri cho productvariantcolor
            model.ProductVariantColor.ColorCode = productVariant.ColorCode;
            model.ProductVariantColor.ProductVariantId = productVariant.Id;
            if (_permissionService.Authorize(StandardPermissionProvider.DisplayPrices))
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        int discount = 0;
                        if (oldPrice > finalPriceWithDiscount)
                        {
                            discount = Convert.ToInt32(100 - (finalPriceWithDiscount * 100 / oldPrice));
                        }
                        model.ProductVariantPrice.Discount = discount;
                    }
                }
            }
            else
        ...
        #region CategoriesManufacture
        [NopHttpsRequirement(SslRequirement.No)]
        public ActionResult ManufactureCategory(int categoryId, int manufactureId, CatalogPagingFilteringModel command)
        {
        ...
        public ActionResult Category(int categoryId, CatalogPagingFilteringModel command)
        {
            ...
            string cacheKey = "Category_Cache_" + categoryId + "_" + orderBy + "_" + price + "_" + pageNumber;
            ...
            CategoryModel model = _cacheManager.Get(cacheKey, () =>
            {
                ...
                //Get all list ppv string
                //IList<string> alreadyFilteredPropertiesOption = model.PagingFilteringContext.PropertiesFilter.GetAlreadyFilteredPropertiesOption(_webHelper);
                IList<int> alreadyFilteredPropertiesOptionIds = new List<int>();

                //Convert list ppv string to list Id
                //foreach (var ppvoption in alreadyFilteredPropertiesOption)
                //{
                ...
                var products = _productService.SearchProducts(out filterableSpecificationAttributeOptionIds, true,
                    ...
                    priceMin: minPriceConverted == null ? 1000 : minPriceConverted,
                    priceMax: maxPriceConverted == null ? int.MaxValue : maxPriceConverted,
                ...
                //model.PagingFilteringContext.PropertiesFilter.PreparePropertiesFilters(categoryId, null, null, _categoryService, _webHelper, _workContext);
                return model;
            });
        ...
        public ActionResult Manufacturer(int manufacturerId, CatalogPagingFilteringModel command)
        {
            ...
            string price = Request["price"];
            string pageNumber = Request["pagenumber"];
            string cacheKey = "Manufacturer_Cache_" + manufacturerId + "_" + orderBy + "_" + price + "_" + pageNumber;
            ...
            ManufacturerModel model = _cacheManager.Get(cacheKey, () =>
                {
        ...
        public ActionResult RelatedProducts(int productId, int? productThumbPictureSize)
        {
            ...
            if (productCategories.Count > 0)
            {
                ...
                if (category != null)
                {
                    ViewBag.CategoryUrl = GetCategoryBreadCrumb(category).FirstOrDefault().GetSeName().ToString();
                }
        ...
        {
            string cacheKey = "RecentlyAddedProducts";
            List<ProductOverviewModel> model = _cacheManager.Get(cacheKey, () =>
            {
        ...
        {
            MiniProfiler.Current.Step("HomepageProducts 1");
            var productHomepages = _cacheManager.Get(ModelCacheEventConsumer.HOMEPAGE_PRODUCTS, () => _productService.GetAllProductsDisplayedOnHomePage());
            //ACL and store mapping
            var products = productHomepages.Where(p => _aclService.Authorize(p) && _storeMappingService.Authorize(p)).ToList();

            MiniProfiler.Current.Step("HomepageProducts 2");
            var model = _cacheManager.Get(ModelCacheEventConsumer.HOMEPAGE_PRODUCTS_MODEL, () => PrepareProductOverviewModels(products, true, true, productThumbPictureSize)
                .ToList());
            MiniProfiler.Current.Step("HomepageProducts 3");
            return PartialView(model);
        }
        ...
        [ChildActionOnly]
        public ActionResult HomepageCollections(int? productThumbPictureSize)
        {
            MiniProfiler.Current.Step("HomepageCollections 1");
            var productCollections = _cacheManager.Get(ModelCacheEventConsumer.HOMEPAGE_COLLECTIONS, () => _productService.GetAllProductCollections());
            //ACL and store mapping
            var products = productCollections.Where(p => _aclService.Authorize(p) && _storeMappingService.Authorize(p)).ToList();
            MiniProfiler.Current.Step("HomepageCollections 2");
            var model = _cacheManager.Get(ModelCacheEventConsumer.HOMEPAGE_COLLECTIONS_MODEL, () => PrepareProductOverviewModels(products, true, true, productThumbPictureSize).ToList());
            MiniProfiler.Current.Step("HomepageCollections 3");
            return PartialView(model);
        }

        public ActionResult ProductCollections(int productId, int? productThumbPictureSize)
        {
            MiniProfiler.Current.Step("ProductCollections_" + productId + " 1");
            var products = new List<Product>();
            //var productCollections = _productService.GetProductsCollectionByProductId1(productId);
            var productCollections = _cacheManager.Get(string.Format(ModelCacheEventConsumer.HOMEPAGE_PRODUCT_COLLECTIONS_IDS_KEY, productId), () => _productService.GetCrossSellProductsByProductId1(productId));
            MiniProfiler.Current.Step("ProductCollections_" + productId + " 2");
            foreach (var product in _productService.GetProductsByIds(productCollections.Select(x => x.ProductId2).ToArray()))
            {
                var variants = _productService.GetProductVariantsByProductId(product.Id);
                //ensure that a product has at least one available variant
                //and has ACL permission and appropriate store mapping
            ...
            MiniProfiler.Current.Step("ProductCollections_" + productId + " 3");
            var model = PrepareProductOverviewModels(products, true, true, productThumbPictureSize).ToList();
            MiniProfiler.Current.Step("ProductCollections_" + productId + " 4");

            return PartialView(model);
        }

        [ChildActionOnly]
        public ActionResult HomepageQuickSearch()
        {
            return PartialView();
        }
        public string ChangeProductVariantColor(int intProductVariantId)
        {
            string price = "";
            string oldprice = "";
            string status = "";
            string pricewithdiscount = "";
            decimal pricevalue, oldpricevalue, pricewithdiscountvalue = 0;
            int discount = 0;
            var productVariant = _productService.GetProductVariantById(intProductVariantId);
            if (productVariant != null)
            {
                if (_permissionService.Authorize(StandardPermissionProvider.DisplayPrices))
                {
                    if (!productVariant.CallForPrice)
                    {
                        ...
                        if (finalPriceWithoutDiscountBase != oldPriceBase && oldPriceBase > decimal.Zero)
                        {
                            oldprice = _priceFormatter.FormatPrice(oldPrice);
                        ...
                        pricevalue = finalPriceWithoutDiscount;
                        pricewithdiscountvalue = finalPriceWithDiscount;

                        if (oldPrice > finalPriceWithDiscount)
                        {
                            discount = Convert.ToInt32(100 - (finalPriceWithDiscount * 100 / oldPrice));
                        }
                        status = productVariant.FormatStockMessage(_localizationService);
                    }
                    else
                        status = "Gọi để biết giá";
                }

                return "{\"id\":\"" + intProductVariantId + "\",\"p\":\"" + price + "\",\"op\":\"" + oldprice + "\",\"dis\":\"" + discount + "\",\"s\":\"" + status + "\",\"promo\":\"" + (productVariant.Promotion != null ? productVariant.Promotion.Replace("\r\n", "<br />") : "") + "\",\"pn\":\"" + productVariant.Name + "\"}";
            }
            return "{\"id\":\"" + 0 + "\",\"p\":\"" + 0 + "\",\"op\":\"" + 0 + "\",\"dis\":\"" + 0 + "\",\"s\":\"" + "" + "\",\"promo\":\"" + "" + "\",\"pn\":\"" + "" + "\"}";
        }
        ...
        public ActionResult ProductsByTag(int productTagId, CatalogPagingFilteringModel command)
        {
            ...
            var model = new ProductConfigModel()
            {
            ...
            var products = _productService.SearchProducts(
                ...
                pageIndex: command.PageNumber - 1,
                pageSize: command.PageSize);
            model.Products = PrepareProductOverviewModels(products, isSearching: true).ToList();
        ...
        {
            var product = _productService.GetProductById(productId);
            if (product == null || product.Deleted || !product.AllowCustomerReviews)
                return InvokeHttp404();
        ...
        public ActionResult AjaxProductReviewsAdd(FormCollection form)
        {
            //bool captchaValid
            ProductReviewsModel model = new ProductReviewsModel();
            model.ProductId = Convert.ToInt32(form["ProductId"]);
            var product = _productService.GetProductById(model.ProductId);
            if (product == null || product.Deleted || !product.Published || !product.AllowCustomerReviews)
            {
                return Json(new { succ = 0, code = 2 });
            }
            //validate CAPTCHA
            //if (_captchaSettings.Enabled && _captchaSettings.ShowOnProductReviewPage && !captchaValid)
            //{
            //    Json(new { succ = 0, code=3 });
            //}

            if (_workContext.CurrentCustomer.IsGuest() && !_catalogSettings.AllowAnonymousUsersToReviewProduct)
            {
                return Json(new { succ = 0, code = 3 });
            }

            if (ModelState.IsValid)
            {
                //save review
                int rating = Convert.ToInt32(form["AddProductReview.Rating"]);
                if (rating < 1 || rating > 5)
                    rating = _catalogSettings.DefaultProductRatingValue;
                bool isApproved = !_catalogSettings.ProductReviewsMustBeApproved;
                ...
                var productReview = new ProductReview()
                {
                    ProductId = product.Id,
                    CustomerId = _workContext.CurrentCustomer.Id,
                    Title = form["AddProductReview.Title"],
                    ReviewText = form["AddProductReview.ReviewText"],
                    Rating = rating,
                    HelpfulYesTotal = 0,
                    HelpfulNoTotal = 0,
                    IsApproved = isApproved,
                ...
                return Json(new { succ = 1 });
            }

            return Json(new { succ = 0, code = 1 });
        }
        [HttpPost]
        ...
        public int CheckProductToCompare(int productId)
        {
            var product = _productService.GetProductById(productId);
            var products = _compareProductsService.GetComparedProducts();
            if (products.Count > 0 && product.ProductCategories.FirstOrDefault().CategoryId != products.FirstOrDefault().ProductCategories.FirstOrDefault().CategoryId)
                return 0;
            return 1;
        }
        //compare products
        public ActionResult AddProductToCompareList(int productId)
        {
            ...
            if (products.Count > 0 && product.ProductCategories.FirstOrDefault().CategoryId != products.FirstOrDefault().ProductCategories.FirstOrDefault().CategoryId)
            {
                return RedirectToAction("ClearCompareList", new { productId = productId });
            }
        ...
        public ActionResult CompareProducts(string productIds)
        {
            ...
            {
                int[] iProductIds = productIds.Split(',').Select(x => Convert.ToInt32(x.Trim())).ToArray();
                if (iProductIds != null && iProductIds.Length > 0)
                {
                    if (iProductIds.Length > 4)
                        iProductIds = iProductIds.Take(4).ToArray();
                    products = _productService.GetProductsByIds(iProductIds);
                }
            }
            else
            ...
            var model = new CompareProductsModel()
            {
                ...
                HtmlCompareProducts = _compareProductsService.GenerateCompare(products)
            };

            PrepareProductOverviewModels(products, prepareSpecificationAttributes: true, isSearching: true)
                .ToList()
                .ForEach(model.Products.Add);
            return View(model);
        }

        public ActionResult ClearCompareList(int? productId = null)
        {
            if (!_catalogSettings.CompareProductsEnabled)
                return RedirectToRoute("HomePage");
            ...
            if (productId != null)
                return RedirectToAction("AddProductToCompareList", new { productId = productId.Value });
        ...
        [ChildActionOnly]
        public ActionResult SearchBoxForCompare()
        {
            var model = new SearchBoxModel()
            {
                AutoCompleteEnabled = _catalogSettings.ProductSearchAutoCompleteEnabled,
        ...
        public ActionResult SearchTermAutoComplete(string term)
        {
            ...
            //products = new PagedList<Product>(products.OrderBy(x => x.ProductCategories.OrderBy(y => y.DisplayOrder).First().DisplayOrder), 0, int.MaxValue, products.Count);
            var models = PrepareProductOverviewModels(products, true, _catalogSettings.ShowProductImagesInSearchAutoComplete, _mediaSettings.AutoCompleteSearchThumbPictureSize, isSearching: true).ToList();
            var result = (from p in models
                          select new
                          {
                              ...
                              price = !string.IsNullOrEmpty(p.ProductPrice.Price) ? p.ProductPrice.Price : p.ProductPrice.OldPrice
