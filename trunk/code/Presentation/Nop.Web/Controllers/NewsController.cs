...
namespace Nop.Web.Controllers
{
    ...
    public partial class NewsController : BaseNopController
    {
        ...
        [NonAction]
        protected void PrepareNewsItemModel(NewsItemModel model, NewsItem newsItem, bool prepareComments, bool preparePictureModel = true,
            int? newsThumbPictureSize = null)
        {
            if (newsItem == null)
            ...
            if (preparePictureModel)
            {
                ...
                int pictureSize = newsThumbPictureSize.HasValue ? newsThumbPictureSize.Value : _mediaSettings.ProductThumbPictureSize;
                //prepare picture model
                model.DefaultPictureModel.FullSizeImageUrl = _pictureService.GetPictureUrl(newsItem.PictureId, 300, true);
                model.DefaultPictureModel.ImageUrl = _pictureService.GetPictureUrl(newsItem.PictureId, pictureSize, true);
                model.DefaultPictureModel.Title = string.Format(_localizationService.GetResource("Media.Product.ImageLinkTitleFormat"), model.Title);
                model.DefaultPictureModel.AlternateText = string.Format(_localizationService.GetResource("Media.Product.ImageAlternateTextFormat"), model.Title);

                //var defaultNewsPictureCacheKey = string.Format(ModelCacheEventConsumer.NEWS_DEFAULTPICTURE_MODEL_KEY, newsItem.Id, pictureSize, true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured());
                //model.DefaultPictureModel = _cacheManager.Get(defaultNewsPictureCacheKey, () =>
                //{
                //    var picture = newsItem.GetDefaultNewsPicture(_pictureService);
        ...
        public ActionResult List(NewsPagingFilteringModel command)
        {
            ...
            string cacheKey = "ListNews_Cache_" + model.WorkingLanguageId + "_" + _storeContext.CurrentStore.Id + "_" + command.PageNumber;


            if (command.PageSize <= 0) command.PageSize = _newsSettings.NewsArchivePageSize;
            if (command.PageNumber <= 0) command.PageNumber = 1;

            var newsItems = _cacheManager.Get(cacheKey, () =>_newsService.GetListNews(_workContext.WorkingLanguage.Id, _storeContext.CurrentStore.Id,
            ...
            model.NewsItems = newsItems
                ...
                {
                    var newsModel = new NewsItemModel();
                    PrepareNewsItemModel(newsModel, x, false, true, 150);
                    return newsModel;
                })
                .ToList();
        ...
        public ActionResult ListByCategory(int categoryId, NewsPagingFilteringModel command)
        {
            ...
            var model = new NewsItemListModel();
            model.WorkingLanguageId = _workContext.WorkingLanguage.Id;
            string cacheKey = "ListByCategory_Cache_" + categoryId + "_" + "_" + command.PageNumber;


            if (command.PageSize <= 0) command.PageSize = _newsSettings.NewsArchivePageSize;
            if (command.PageNumber <= 0) command.PageNumber = 1;

            var newsItems = _cacheManager.Get(cacheKey, () => _newsService.GetNewsByIds(_newsCategoryService.GetNewsItemCategoriesByCategoryId( 
                categoryId, command.PageNumber - 1, command.PageSize).Select(x => x.NewsId).ToArray()));
            model.PagingFilteringContext.LoadPagedList(new PagedList<NewsItem>(newsItems,command.PageNumber - 1, command.PageSize));

            ViewBag.CategoryName = _cacheManager.Get("CategoryName_Cache_" + categoryId, () => _newsCategoryService.GetNewsCategoryById(categoryId).Name);
            model.NewsItems = newsItems
                .Select(x =>
                {
                    var newsModel = new NewsItemModel();
        ...
        public ActionResult TopHotNews(NewsPagingFilteringModel command)
        {
            if (!_newsSettings.Enabled)
                return RedirectToRoute("HomePage");
            ...
            var model = new NewsItemListModel();
            model.WorkingLanguageId = _workContext.WorkingLanguage.Id;
            string cacheKey = "TopHotNews_Cache_0_" + model.WorkingLanguageId + "_" + _storeContext.CurrentStore.Id + "_5";

            var newsItems = _cacheManager.Get(cacheKey, () => _newsService.GetTopFeaturedNews(_workContext.WorkingLanguage.Id, _storeContext.CurrentStore.Id, 0, 5));

            model.NewsItems = newsItems
                .Select(x =>
                {
                    var newsModel = new NewsItemModel();
                    PrepareNewsItemModel(newsModel, x, false, true, 80);
                    return newsModel;
                })
                .ToList();
            if (command.PageNumber > 1)
                return PartialView(new NewsItemListModel());
            return PartialView(model);
        }

        public ActionResult MostReadNews()
        {
            if (!_newsSettings.Enabled)
                return RedirectToRoute("HomePage");

            string cacheKey = "MostReadNews_Cache";
            var model = new NewsItemListModel();
            var newsItems = new List<NewsItem>();
            var newsViewTrackings = _cacheManager.Get(cacheKey, () => _newsService.GetTopNewsViewTracking());
            foreach (var newsItem in _newsService.GetNewsByIds(newsViewTrackings.Select(x => x.NewsItemId).ToArray()))
        ...
        public ActionResult MostReadNewsByCategory(int categoryId)
        {
            if (!_newsSettings.Enabled)
                return RedirectToRoute("HomePage");

            string cacheKey = "MostReadNewsByCategory_Cache";
            var model = new NewsItemListModel();
            var newsItems = new List<NewsItem>();
            var newsViewTrackings = _cacheManager.Get(cacheKey, () => _newsService.GetTopNewsViewTrackingByCategory(categoryId));
            foreach (var newsItem in _newsService.GetNewsByIds(newsViewTrackings.Select(x => x.NewsItemId).ToArray()))
                newsItems.Add(newsItem);
        ...
        public ActionResult NewsItem(int newsItemId)
        {
            ...
            // Check if newsItem is first viewed by user or not
            string sDetailPageViewCookieName = string.Format("cc_newsItem_page_view_turn_{0}", newsItemId);
            ViewBag.FirstPageView = false;
            if (Request.Cookies[sDetailPageViewCookieName] == null)
            {
                ViewBag.FirstPageView = true;
                HttpCookie ckPageViewCookie = new HttpCookie(sDetailPageViewCookieName);
                ckPageViewCookie.Value = "NOT_FIRST";
                Response.Cookies.Add(ckPageViewCookie);
            }
        ...
        public ActionResult TrackNewsView(int iNewsId)
        {
            if (Request != null)
            {
                _newsService.TrackingNewsView(iNewsId);
            }
            byte[] b = new byte[] { 0 };
            return new FileStreamResult(new System.IO.MemoryStream(b), "image/jpeg");
        }

        [ChildActionOnly]
        public ActionResult RelatedNews(int newsItemId)
        {
            var model = new NewsItemListModel();
            var newsItems = new List<NewsItem>();
            var relatedNews = _newsService
                .GetRelatedNewsByNewsId1(newsItemId);
            foreach (var newsItem in _newsService.GetNewsByIds(relatedNews.Select(x => x.NewsId2).ToArray()))
            {
                newsItems.Add(newsItem);
            }
